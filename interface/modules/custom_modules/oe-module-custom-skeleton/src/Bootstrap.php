...
class Bootstrap
{
    ...
    public function __construct(EventDispatcherInterface $eventDispatcher, ?Kernel $kernel = null)
    {
        ...
        $this->globalsConfig = new GlobalConfig($globalArray);
    ...
    {
        $this->eventDispatcher->addListener(UserCreatedEvent::EVENT_HANDLE, [$this, 'printUserdetails']);
    }

    public function signResultEvent()
    {
        $this->eventDispatcher->addListener(ProcedureSignResultsEvent::EVENT_HANDLE, [$this, 'createServiceRequest']);
    }

    public function createServiceRequest(ProcedureSignResultsEvent $event)
    {
        
        $service_url = 'http://localhost:9081/api/v1/notify';
        $event_data = $event->getResultData();
        
        $this->curlRequest($service_url,$event_data,ProcedureSignResultsEvent::EVENT_HANDLE);
        
    }
    private function curlRequest($service_url,$event_data,$event_name)
    {
        $curl = curl_init($service_url ."/". $event_name);
    ...
    public function printUserdetails(UserCreatedEvent $event)
    {
        $service_url = 'https://usercreate.requestcatcher.com/test';
